...
def generate_edges(n, phi):
    ...
    z = np.full_like(theta, np.sin(phi))
    
    cone_edges = np.column_stack((x, y, z))
    # ------------------------------------------------
...
def compute_normals(cone_edges):
    ...
    # FILL WITH YOUR CODE
    vi = cone_edges
    vi_p_1 = np.roll(cone_edges,shift=-1,axis=0)
    facet_normals = np.cross(vi,vi_p_1)
    # ------------------------------------------------
...
def compute_minimum_distance_from_facet_normals(a, facet_normals):
    ...
    # ------------------------------------------------
    # FILL WITH YOUR CODE
    minimum_distance = np.min(np.dot(facet_normals, a) / np.linalg.norm(facet_normals, axis=1))
    # TODO: Account for case 3
    # ------------------------------------------------
...
def check_is_interior_point(a, n, phi):
    ...
    is_interior_point = minimum_distance > 0
    is_interior_point = bool(is_interior_point)
    # ------------------------------------------------
